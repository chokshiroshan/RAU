...
export async function searchUnified(query, filters, cachedSettings = null) {
  ...
  if (!query || query.trim() === '') {
    logger.debug('UnifiedSearch', 'Empty query, returning empty')
    return []
  }
  ...
  if (calculatorResult !== null) {
    logger.debug('UnifiedSearch', 'Calculator result:', calculatorResult)
    return [{
      type: 'calculator',
  ...
  try {
    ...
    if (activeFilters.tabs && settings && settings.searchTabs !== false) {
      logger.debug('UnifiedSearch', 'Tab search enabled')
      searchPromises.push(
    ...
    if (activeFilters.files && settings && settings.searchFiles !== false) {
      logger.debug('UnifiedSearch', 'File search enabled')
      searchPromises.push(
    ...
    if (allResults.length > 0) {
      ...
      logger.debug('UnifiedSearch', `After fuzzy search: ${fuzzyResults.length} results`)
    }
    ...
    logger.error('UnifiedSearch', 'Unexpected error:', error)
    return []
  }
}
...
export async function searchTabs(query) {
  ...
  try {
    ...
    logger.error('UnifiedSearch', 'Tab search error:', error)
    return []
  }
}
